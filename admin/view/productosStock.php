...
<body>
    ...
    <?php
        ...
        $num_rows = count($productos->listarProductosSinStock());
        $num_register = 9;
        $num_pages = ceil($num_rows/$num_register);

        if(!$_GET['page']){
            header('location: http://localhost/elitesac/admin/view/productosStock.php?page=1');
        }

        if($_GET['page'] < 1 || $_GET['page'] > $num_pages){
            header('location: http://localhost/elitesac/admin/view/productosStock.php?page=1');
        }
        // listarProductosSinStockLimit
    ?>
    <div class="container-fluid p-0">
        <div class="d-flex">
            <?php include 'navegacion.php'; ?>
            <div class="float-end w-100 p-4" style="margin-left:11rem">
                <h3 class="h3 text-center mb-3">Productos Sin Stock</h3>
                        ...
                        <tbody class="table-sm align-middle">
                            <?php
                                ...
                                if($num_rows >= 10){
                                    ...
                                    foreach ($productos->listarProductosSinStockLimit($iniciar, $num_register) as $dato) {
                                        echo "
                                        <tr>
                                        <td>
                                    ...
                                    foreach ($productos->listarProductosSinStock() as $dato) {
                                        echo "
                                        <tr>
                                            <td>
                                                <img style='width:70px;height:70px;object-fit:contain' src='../../images/images-products/".$dato->imagen."' alt=''>
                    ...
                    if($num_rows >= 10){ ?>
                        <ul class="pagination mx-auto">
                            <li class="page-item <?=($_GET['page']<=1) ? 'disabled' : ''?>">
                                <a class="page-link shadow-none" href="http://localhost/elitesac/admin/view/productosStock.php?page=<?=$_GET['page']-1?>">Anterior</a>
                            </li>
                            
                            <?php for ($i=1; $i <= $num_pages ; $i++) { ?>
                                <li class="page-item <?=($_GET['page'] == $i) ? 'active':''?>"><a class="page-link shadow-none" href="http://localhost/elitesac/admin/view/productosStock.php?page=<?=$i?>"><?=$i?></a></li>
                            <?php } ?>

                            <li class="page-item <?=($_GET['page']>=$num_pages) ? 'disabled' : ''?>">
                                <a class="page-link shadow-none" href="http://localhost/elitesac/admin/view/productosStock.php?page=<?=$_GET['page']+1?>">Siguiente</a>
                            </li>
                        </ul>
                <?php } ?>
            </div>
