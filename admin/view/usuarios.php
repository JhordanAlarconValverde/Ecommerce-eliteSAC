...
<body>
    <?php
        ...
        if(!isset($_SESSION['empleado'])){
            header("location: http://localhost/elitesac/admin/index.php");
        }
        ...
        $num_rows = count($usuarios->listarUsuarios());
        $num_register = 9;
        $num_pages = ceil($num_rows/$num_register);

        if(!$_GET['page']){
            header('location: http://localhost/elitesac/admin/view/usuarios.php?page=1');
        }
    ...
    <div class="container-fluid p-0">
        <div class="d-flex">
            ...
            <div class="float-end border border-danger w-100 p-4" style="margin-left:11rem">
                <h3 class="h3 text-center mb-3">Lista de Usuarios</h3>
                ...
                <div class="table-table-responsive-md">
                    <table class="table table-hover text-center">
                        ...
                        <tbody class="table-sm align-middle">
                            <?php
                                ...
                                foreach ($usuarios->listarUsuariosLimit($iniciar, $num_register) as $dato) {
                                    echo "
                                    <tr>
                                    <td>
                                    ...
                                    <td>
                                        <div class='btn-group dropstart'>
                                            ...
                                            <ul class='dropdown-menu mx-auto'>
                                                ...
                                                <li class='dropdown-item'><a href='../controller/adminController.php?idEliminar={$dato->id}' class='btn btn-sm btn-danger w-100 shadow-none'><i class='fa-solid fa-trash-can'></i>&nbsp;&nbsp;Eliminar</a></li>                                    
                                            </ul>
                                        </div>
                    ...
                    <ul class="pagination mx-auto">
                        <li class="page-item <?=($_GET['page']<=1) ? 'disabled' : ''?>">
                            <a class="page-link shadow-none" href="http://localhost/elitesac/admin/view/usuarios.php?page=<?=$_GET['page']-1?>">Anterior</a>
                        </li>
                        
                        <?php for ($i=1; $i <= $num_pages ; $i++) { ?>
                            <li class="page-item <?=($_GET['page'] == $i) ? 'active':''?>"><a class="page-link shadow-none" href="http://localhost/elitesac/admin/view/usuarios.php?page=<?=$i?>"><?=$i?></a></li>
                        <?php } ?>

                        <li class="page-item <?=($_GET['page']>=$num_pages) ? 'disabled' : ''?>">
                            <a class="page-link shadow-none" href="http://localhost/elitesac/admin/view/usuarios.php?page=<?=$_GET['page']+1?>">Siguiente</a>
                        </li>
                    </ul>
            </div>
        </div>
